...
class CI_Session
{
    ...
    function __construct()
    {
        ...
        $this->lifetime = $this->object->config->item('sess_expiration');
        $this->sess_id_ttl = $this->object->config->item('sess_time_to_update');
        $this->match_ip = $this->object->config->item('sess_match_ip');
        $this->match_useragent = $this->object->config->item('sess_match_useragent');
        $this->useDB = $this->object->config->item('sess_use_database');
        $this->sess_table = $this->object->config->item('sess_table_name');
    ...
    function _sess_run()
    {
        ...
        if (!is_numeric($this->lifetime) || $this->lifetime <= 0) {
            $this->lifetime = (60 * 60 * 24 * 365 * 2);
        }

        // if no session ID regeneration time set in config, set to 30 minutes
        if (!is_numeric($this->sess_id_ttl) || $this->sess_id_ttl <= 0) {
            $this->sess_id_ttl = 1800;
        }
    ...
    function _open()
    {
        if ($this->sess_db = mysql_connect($this->object->db->hostname,
                                            $this->object->db->username,
                                            $this->object->db->password)) {
            return mysql_select_db($this->object->db->database, $this->sess_db);
        }
    ...
    {
        if ($this->sess_db) {
            return mysql_close($this->sess_db);
        }
    ...
    function _read($id)
    {
        ...
        $sql = "SELECT session_data FROM $this->sess_table WHERE session_id = '$id'";

        if ($result = mysql_query($sql, $this->sess_db)) {
    ...
    function _write($id, $data)
    {
        ...
        $sql = "REPLACE INTO $this->sess_table VALUES ('$id', '$access', '$data')";

        return mysql_query($sql, $this->sess_db);
    }
    ...
    function _destroy($id)
    {
        ...
        $sql = "DELETE FROM $this->sess_table WHERE session_id = '$id'";

        return mysql_query($sql, $this->sess_db) or die("failed to delete<br>");
    }
    ...
    function _gc($max)
    {
        ...
        $sql = "DELETE FROM $this->sess_table WHERE session_last_access < '$old'";
