...
//        $db = new object();
	...
	$s = $this->uri->segment('s'); /*empty($getData['s']) ? '' : $getData['s'];*/
//        var_dump($s, $s != '', $s == '', $s== false);
	...
	$t['total'] = 0;
	$t['list'] = array();
	$q = $this->tinylist->executeQuery("SELECT *, duedate IS NULL AS ddn FROM {$db->prefix}todolist $inner WHERE 1=1 $sqlWhere $sqlSort");

        foreach ($q AS $r) {
            $t['total']++;
            $t['list'][] = $this->tinylist->prepareTaskRow($r);
        }
